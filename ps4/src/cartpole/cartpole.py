...
def choose_action(state, mdp_data):
    ...
    Ps0 = transition_probs[state, :, 0]
    action_val_0 = Ps0.dot(value)
    Ps1 = transition_probs[state, :, 1]
    action_val_1 = Ps1.dot(value)
    
    if(action_val_0 > action_val_1):
        action = 0
    elif(action_val_0 < action_val_1):
        action = 1
    else: 
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for state in range(num_states):
        if((reward_counts[state, 1] > 0) and (reward_counts[state, 0] > 0)):
            reward[state] = -1
            
        for action in range(2):
            if(not (transition_counts[state, :, action] == np.zeros((num_states))).all()):
                combo_count = np.sum(transition_counts[state, :, action])
                transition_probs[state, :, action] = transition_counts[state, :, action] / combo_count
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    iters = 0
    while(True):
        old_value = np.copy(mdp_data['value'])
        for state in range(mdp_data['num_states']):
            Ps0 = mdp_data['transition_probs'][state, :, 0]
            action_val_0 = gamma * Ps0.dot(mdp_data['value'])
            Ps1 = mdp_data['transition_probs'][state, :, 1]
            action_val_1 = gamma * Ps1.dot(mdp_data['value'])
            mdp_data['value'][state] = mdp_data['reward'][state] + max(action_val_0, action_val_1)
        if(np.max(mdp_data['value'] - old_value) < tolerance):
            return iters==0
        iters += 1
    # *** END CODE HERE ***
