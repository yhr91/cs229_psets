...
def softmax(x):
    ...
    x -= maxvec.reshape((maxvec.shape[0], 1))
    sumvec = np.sum(np.exp(x), axis=1)
    return np.exp(x) / sumvec.reshape((sumvec.shape[0], 1))
    # *** END CODE HERE ***
...
def CEloss(scores, labels):
    N = scores.shape[0]
    loss = np.zeros((N,))
    for i in range(N):
        loss[i] -= np.dot(labels[i], np.log(scores[i]))
    return loss
...
def forward_prop(data, labels, params):
    ...
    output = softmax(np.matmul(hidden, W2) + b2)
    avgloss = np.sum(CEloss(output, labels))/output.shape[0]
    
    return hidden, output, avgloss
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    grads = {}
    W1, b1, W2, b2 = params['W1'], params['b1'], params['W2'], params['b2']
    hidden, output, __ = forward_prop_func(data, labels, params)
    ...
    med = np.matmul(data, W1) + b1
    dmed = dhidden*sigmoid(med)*(1.0-sigmoid(med))
    
    dW1 = np.dot(data.T, dmed)
    db1 = np.sum(dmed, axis=0)
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    """
    # *** START CODE HERE ***
    W1, __, W2, __ = params['W1'], params['b1'], params['W2'], params['b2']
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for batch in range(num_batches):
        ...
        grads = backward_prop_func(train_data[left:right], train_labels[left:right], params, forward_prop_func)
        for param in params:
