...
def init_centroids(num_clusters, image):
    ...
    # *** START YOUR CODE ***
    rows = np.random.randint(0, image.shape[0], num_clusters)
    cols = np.random.randint(0, image.shape[1], num_clusters)
    centroids_init = image[rows, cols] 
    # *** END YOUR CODE ***
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for i in range(max_iter):
        ...
        centroid_map = np.zeros((image.shape[0], image.shape[1]))
        centroid_counts = np.zeros((centroids.shape[0]))
        
        # update pixels
        for row in range(image.shape[0]):
            for col in range(image.shape[1]):
                pixelcopies = np.zeros_like(centroids)
                pixelcopies += image[row, col]
                distances = np.square(np.sum(pixelcopies - centroids, axis=1))
                best_centroid = np.argmin(distances)
                centroid_map[row, col] =  best_centroid
                centroid_counts[best_centroid] += 1
        
        # update centroids
        new_centroids = np.zeros_like(centroids, dtype=float)
        
        for row in range(centroid_map.shape[0]):
            for col in range(centroid_map.shape[1]):
                centroid = centroid_map[row, col]
                count = centroid_counts[int(centroid)]
                new_centroids[int(centroid)] += image[row, col]/centroid_counts[int(centroid)]
        
        new_centroids = new_centroids.astype(int)        
        
        if(np.sum(centroids==new_centroids)/centroids.shape[1]==centroids.shape[0]):
...
def update_image(image, centroids):
    ...
    for row in range(image.shape[0]):
        for col in range(image.shape[1]):
            ...
            distances = np.square(np.sum(pixelcopies - centroids, axis=1))
            best_centroid = np.argmin(distances)
            image[row, col] =  centroids[best_centroid]
    # *** END YOUR CODE ***
